    ...
    <div className="anim-fade-up">
      ...
      <div className="grid-4 stagger" style={{ marginBottom: 24 }}>
        ...
        <StatCard
          ...
          icon={<DollarSign size={22} />} color="var(--green)" bg="var(--green-l)" accentColor="#10b981"
          sub="at cost price"
        />
        <StatCard
          label="Low Stock" value={loading ? '—' : lowStock.length}
          icon={<AlertTriangle size={22} />} color="var(--amber)" bg="var(--amber-l)" accentColor="#f59e0b"
          sub="items need reorder"
        />
        <StatCard
          label="Out of Stock" value={loading ? '—' : outStock.length}
          icon={<TrendingDown size={22} />} color="var(--red)" bg="var(--red-l)" accentColor="#ef4444"
          ...
          <div style={{ padding: '18px 22px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 11.5, color: 'var(--ink-3)' }}>Latest stock movements</p>
              </div>
            </div>
          ...
          <table className="table">
            ...
            <tbody>
              {loading
                ...
                : txns.length === 0
                ? <tr><td colSpan={4} style={{ textAlign: 'center', padding: '40px 16px', color: 'var(--ink-3)' }}>No transactions yet</td></tr>
                : txns.map(tx => (
                    <tr key={tx.id}>
                      <td>
                      ...
                      <td style={{ fontWeight: 800, color: txColor(tx.transaction_type), fontSize: 15, fontFamily: 'var(--font-head)' }}>
                        ...
                        <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--ink-3)', marginLeft: 3 }}>{tx.products?.unit}</span>
                      </td>
        ...
        <div className="card" style={{ overflow: 'hidden' }}>
          <div style={{ padding: '18px 22px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 11.5, color: 'var(--ink-3)' }}>Items needing attention</p>
              </div>
            </div>
          ...
          {loading
            ...
            ? <div style={{ padding: '52px 20px', textAlign: 'center' }}>
                ...
                <p style={{ fontSize: 12.5, color: 'var(--ink-3)', marginTop: 4 }}>No action needed right now.</p>
              </div>
            : [...outStock, ...lowStock].slice(0, 7).map(p => (
              <div key={p.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 22px', borderBottom: '1px solid var(--border)' }}>
                <div style={{ minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: 11.5, color: 'var(--ink-3)', fontFamily: 'var(--mono)' }}>{p.sku}</p>
                </div>
