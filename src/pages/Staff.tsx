...
import { Plus, Edit2, UserX, UserCheck, Shield, User as UserIcon, AtSign } from 'lucide-react'
    ...
    <div className="anim-fade-up">
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ color: 'var(--ink-3)', fontSize: 13.5, marginTop: 3 }}>{rows.filter(r=>r.is_active).length} active · {rows.filter(r=>!r.is_active).length} inactive</p>
        </div>
      ...
      <div className="grid-2" style={{ marginBottom: 18 }}>
        {[
          { role: 'Admin', Icon: Shield, color: 'var(--gold)', bg: 'var(--gold-l)', desc: 'Full access — staff, reports, categories, delete operations.' },
          { role: 'Staff', Icon: UserIcon, color: 'var(--ink-3)', bg: 'var(--bg)', desc: 'Standard access — inventory, transactions, suppliers (view only for delete).' },
          ...
          <div key={role} className="card-inset" style={{ padding: '12px 16px', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
            ...
            <div><p style={{ fontWeight: 700, fontSize: 13, color: 'var(--ink)', marginBottom: 2 }}>{role}</p><p style={{ fontSize: 12.5, color: 'var(--ink-3)', lineHeight: 1.5 }}>{desc}</p></div>
          </div>
        ))}
      </div>
                ...
                : rows.map(s => {
                  ...
                  return (
                    <tr key={s.id} style={{ opacity: s.is_active ? 1 : .45 }}>
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          <div>
                            ...
                            <p style={{ fontSize: 11.5, color: 'var(--ink-3)' }}>{s.email || 'No email'}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td style={{ fontSize: 12.5, color: 'var(--ink-3)' }}>{new Date(s.created_at).toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' })}</td>
                      <td>
                        <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
                          <button className="btn-icon" onClick={() => open(s)}><Edit2 size={13} /></button>
                          <button className={`btn-icon ${s.is_active ? 'danger' : ''}`} onClick={() => setToggleTarget(s)}>
                            {s.is_active ? <UserX size={13} /> : <UserCheck size={13} style={{ color: 'var(--green)' }} />}
                          </button>
                        </div>
                      </td>
                    </tr>
            ...
            {!editing && (
              <Field label="Username (used to log in)" required>
                <div style={{ position: 'relative' }}>
                  <span style={{ position: 'absolute', left: 11, top: '50%', transform: 'translateY(-50%)', fontFamily: 'var(--mono)', fontSize: 13, color: 'var(--ink-3)' }}>@</span>
